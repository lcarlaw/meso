...
def sharppy_calcs(**kwargs):
    ...
    rm5 = {
        '0-6kmAgl': (np.zeros_like(mucape), np.zeros_like(mucape)),
        'BlkShr': (np.zeros_like(mucape), np.zeros_like(mucape)),
    }
    lm5 = {}
    ...
    for j in range(tmpc.shape[1]):
        for i in range(tmpc.shape[2]):
            ...
            rm5['BlkShr'][0][j,i], rm5['BlkShr'][1][j,i] = winds.wind_shear(prof,
                                                                            pbot=sfc,
                                                                            ptop=p6km)
            rm5['0-6kmAgl'][0][j,i], rm5['0-6kmAgl'][1][j,i] = winds.mean_wind(prof,
                                                                               pbot=sfc,
                                                                               ptop=p6km)
            t3 = time()
    ...
    for j in range(tmpc.shape[1]):
        for i in range(tmpc.shape[2]):
            prof = profile.create_profile(pres=pres[:,j,i], tmpc=tmpc[:,j,i],
                                          hght=hght[:,j,i], dwpc=dwpc[:,j,i],
                                          wspd=wspd[:,j,i], wdir=wdir[:,j,i])
            esrh[j,i] = winds.helicity(prof, ebot[j,i], etop[j,i], stu=rm5['u'][j,i],
                                       stv=rm5['v'][j,i])[0]
            estp[j,i] = params.stp_cin(mlcape[j,i], esrh[j,i], eshr[j,i],
                                       mllcl[j,i], mlcin[j,i])

            p1km = interp.pres(prof, interp.to_msl(prof, 1000.))
            srh1 = winds.helicity(prof, ebot[j,i], p1km, stu=rm5['u'][j,i],
                                       stv=rm5['v'][j,i])[0]
